...
export const toolDefinition = {
  ...
  description:
    'Upload a file to Baklib DAM (Digital Asset Management). Response includes JSON:API data.id as id, and attributes.iid as iid. For BKE Markdown images/links, use dam-id=iid (NOT id)—see baklib-bke-markdown §1.4.',
  inputSchema: {
    type: 'object',
    properties: {
      file_path: {
...
export async function handleTool(args) {
  ...
  try {
    ...
    /** BKE Markdown `dam-id` must match this (workbench address bar), not JSON:API `data.id`. */
    const iid = attrs?.iid;
