...
export const toolDefinition = {
  ...
  description:
    'Update a page in a Baklib site. Responses return template_variables richtext as BKE Markdown when present (body_format=markdown). Writes still follow Dynamic Form storage rules (often HTML for richtext).',
  inputSchema: {
    type: 'object',
    properties: {
      site_id: {
      ...
      template_variables: {
        type: 'object',
        description:
          'Template variables. GET returns richtext as Markdown; submit updates in the format the API expects (often HTML), not necessarily raw Markdown.',
      },
      published: {
        type: 'boolean',
